...
def test_schema_false_return(exp_res: dict, inp: MetadataSchemaReader) -> None:
    # assert exp_res == MetadataSchemaReader(lhs).searchedSchema
    assert inp == exp_res
...
def test_raises(x) -> None:
    with pytest.raises(TypeError):
        _ = DicomReader(file1)

# monkeypatch for testing outputs and logs
'''
